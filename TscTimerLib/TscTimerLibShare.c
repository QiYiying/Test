...
UINT32
FindPmTmrAddress (VOID)
{
  //  CONST ACPI_FADT * pFadt;
  CONST EFI_ACPI_1_0_FIXED_ACPI_DESCRIPTION_TABLE *pFadt;

  //  pFadt = (ACPI_FADT *)LocateTable ( FADT_SIGNATURE );
  pFadt = (EFI_ACPI_1_0_FIXED_ACPI_DESCRIPTION_TABLE *)LocateTable ( FADT_SIGNATURE );
  if (pFadt == NULL) {
    return 0;
  }
  return pFadt->PmTmrBlk;
}
...
//@LENOVO_CUSTOMZE+
  ...
  if ((Ebx == 0x68747541) &&  // "Auth"
      ...
      (Ecx == 0x444D4163))    // "cAMD"
  {
    //    TimerAddr = 0x808;  // AMD chipset

    //
    // Get PM_TMR address from FADT.
    //
    TimerAddr = FindPmTmrAddress ();
    if (TimerAddr == 0) {
      TimerAddr = 0x808;  // Default for AMD chipset
    }
    Print (L"TimerAddr = 0x%x\n", TimerAddr);
